...
export function registerMapiCodeMode(server: McpServer, env: Env, props: Props): void {
  ...
  registerCodeModeVariant(server, env, props, {
    ...
    accessMode,
    description:
      "This is the Knock Management API (control plane). Use it to define and configure workflows, channels, templates, broadcasts, guides, layouts, partials, commits, environments, and other account resources. Do not use it to trigger workflows or manage environment-scoped runtime data such as users, tenants, objects, preferences, schedules, subscriptions, or messages; use API code mode (`search_api`, `execute_api_read`, and `execute_api_write` when API Manage is enabled) for those operations.",
